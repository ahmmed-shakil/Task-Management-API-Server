...
class SocketService {
  ...
  private authenticateSocket(socket: AuthenticatedSocket, token: string) {
    try {
      ...
      console.log(
        `✅ Socket authenticated: ${socket.id} -> User: ${decoded.email}`
      );
    } catch (error) {
  ...
  emitTaskCompleted(projectId: string, task: any) {
    if (!this.io) return;
  ...
  // File upload events
  emitFileUploadProgress(userId: string, fileId: string, progress: number) {
    if (!this.io) return;
  ...
  emitFileUploaded(taskId: string, file: any) {
    if (!this.io) return;
